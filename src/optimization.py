...
def lsq_lma_operator(
    ...
    """
    Levenberg-Marquardt implementation for least-squares fitting of non-linear functions with operator Jacobian
...
if __name__ == '__main__':
    ...
    print('Linear Test with Full Jacobian')
    print(lsq_lma(p = torch.randn(m), function = res, jac_function = Jac, verbose = True))

    def JacOP(x):
        assert x.shape == (m,)
        return LinearOperator((m,), lambda x : A@x)
    
    print('Linear Test with Operator Jacobian')
    print(lsq_lma_operator(p = torch.randn(m), function = res, jac_function = JacOP, verbose = True))
